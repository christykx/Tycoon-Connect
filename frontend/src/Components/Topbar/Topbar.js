...
export default function Topbar({ socket }) {
    ...
    const handleSubmit = (e) => {
        ...
        console.log("ABCDRFGHIJK");

        makeRequest.post('/users/post', formdata).then((response) => {
            console.log("YUpppppppppppppppppp");

            if (response.status) {
                console.log(response, "Yes foundddddddddddd");
                console.log(response.data, "Dataaaaa reached");
            ...
            console.log("-----errrrr--- errrr found", err.response.data);
    ...
    useEffect(() => {
        ...
        makeRequest.get('/users/getnotifications').then((response) => {
        ...
        makeRequest.get(`/users/getnotificationlength/${id}`).then((response) => {
            ...
            if (response.status) {
                ...
                // setnotificationlength((prev) => [...prev, response?.data])
                // setprofile(response?.data)
        ...
        //     axios.get(`http://localhost:3001/users/getotherprofile/${Otheruserid._id}`).then((response) => {
                    ...
                    {usernames && usernames.map((user, index) => {
                        ...
                        return <div key={user._id}  >
                            ...
                            {(search.trim().toLowerCase().includes(`${user.username}`.toLowerCase())) &&
                                ...
                                //    console.log("Kittiiii",`${user.username}`)    
                                // <Card style={{ border: '2px solid grey' }} onClick={viewSingleUser(user._id)} >
                                ...
                                <Card style={{ border: '2px solid grey' ,cursor:'pointer'}}  onClick={() => {
                                    ...
                                    <Stack direction="row" >
                                        ...
                                        <Avatar src={`${config.ServerURL}/uploads/${user?.ProfileData.profilePicture}`} />
                ...
                <Box sx={{ display: { xs: 'none', md: 'flex' } }}>
                    ...
                    <IconButton onClick={() => { navigate('/profile') }}>
                        <StyledBadge
                            ...
                            variant="dot"
                        >
                            <Avatar src={`${config.ServerURL}/uploads/${profile}`} />
                        </StyledBadge>
                    </IconButton>
